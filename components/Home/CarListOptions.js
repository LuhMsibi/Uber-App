  ...
  return (
    <div className='mt-5 p-5 overflow-auto h-[250px]'>
       ...
       {selectedCar?.name?<div className='flex justify-between fixed bottom-5 bg-white p-3 shadow-xl w-full md:w-[30%] border-[1px] items-center rounded-lg'>
            ...
            <button className='bg-black text-white rounded-lg text-center p-3' onClick={()=>router.push('/payment?amount='+ ((selectedCar.amount*distance * 20)).toFixed(2))}>Request {selectedCar.name}</button>
        </div>: null}
    </div>
  )
}
